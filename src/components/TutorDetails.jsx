...
const TutorDetails = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://tutor-lagbe-server.vercel.app/bookedTutors?userEmail=${user?.email}&tutorId=${_id}`
      )
      .then((res) => {
  ...
  const handleBookTutor = async (id) => {
    ...
    if (user && user.email) {
      ...
      toast.promise(
        axios.post(
          "https://tutor-lagbe-server.vercel.app/bookedTutors",
          bookedTutor
        ),
        {
          loading: "Booking Tutor...",
          success: <b>Successfully booked tutor!</b>,
  ...
  const handleReview = (id) => {
    if (user && user.email) {
      ...
      axios
        .post("https://tutor-lagbe-server.vercel.app/reviews", reviewData)
        .then((res) => {
        ...
        .catch((error) => {
          ...
          Swal.fire({
            ...
            icon: "error",
            title:
              "Unable to submit the review. You can review a tutor only Once.",
  ...
  return (
    <div className="">
      <div className="h-[250px] md:rounded-bl-[60px] md:rounded-br-[60px] md:w-11/12 mx-auto bg-[#540654] relative">
        <h1 className="text-3xl lg:text-4xl font-bold text-center text-white pt-12">
          Tutor Details
        </h1>
      </div>
      <motion.div
      ...
      >
        ...
        <div className="md:w-1/4  rounded-xl bg-base-200 shadow-md">
          <div className="p-3">
            <h1 className="text-center underline font-thin text-2xl my-6">
              Profile
            </h1>
            ...
            <table className="table">
              <tbody>
                <tr className="border-t">
                  <td className="w-1/3 font-bold text-md">
                    Expected Minimum Salary
                  </td>
                  <td className="w-2/3 font-semibold text-xl">
                    {price} <span className="text-xs">tk/mo</span>
                  </td>
                </tr>
                <tr>
                  <td className="w-1/3 font-bold text-md">
                ...
                <tr>
                  ...
                  <td className="w-2/3 text-[#540654] text-lg font-bold">
                    {language} Lessons
                  </td>
                </tr>
                <tr>
          ...
          {isBooked ? (
            <button
              ...
              className={`btn  mt-5 w-full ${
                isDisabled ? "bg-base-200" : "bg-[#00d4ff] "
              }  `}
            >
              Review This Tutor
            </button>
            ...
            <button
              onClick={() => handleBookTutor(_id)}
              disabled={isDisabled}
              className={`btn  mt-5 w-full ${
                isDisabled
                  ? "bg-base-200  cursor-auto"
                  : "bg-gradient-to-r  from-[#540654] via-[#cc0d85] to-[#00d4ff]"
              }  text-white`}
            >
              Book This Tutor
            </button>
